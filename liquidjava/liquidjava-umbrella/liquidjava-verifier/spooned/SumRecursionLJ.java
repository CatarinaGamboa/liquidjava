...
@liquidjava.specification.RefinementAlias("Nat(int x) {x >= 0}")
@liquidjava.specification.RefinementAlias("GreaterEqualThan(int x, int y) {x >= y}")
public class SumRecursionLJ {
    // recursion + ifs -> Example tutorial refinement types
    @liquidjava.specification.Refinement("Nat(_) && _ >= n")
    ...
    @liquidjava.specification.Refinement("_ >= 1 && GreaterEqualThan(_, n)")
    public static int fibonacci(@liquidjava.specification.Refinement("Nat(n)")
    int n) {
        if (n <= 1)
            return 1;
    ...
    @liquidjava.specification.Refinement("(n < 0) ? (_ == -n) : (_ == n)")
    ...
    public static double getGrade(int exam, int essay, int participation, int bonus) {
        @liquidjava.specification.Refinement("_ == 0.5")
        double valueExam = 0.5;
        @liquidjava.specification.Refinement("_ == 0.25")
        double valueEssay = 0.25;
        @liquidjava.specification.Refinement("_ == 0.12")
        double valueParticipation = 0.12;
        @liquidjava.specification.Refinement("(valueExam + valueEssay + valueParticipation + _ ) == 1")
