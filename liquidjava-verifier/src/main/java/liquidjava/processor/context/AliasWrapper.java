...
public class AliasWrapper {
  ...
  public AliasWrapper(
      AliasDTO a, Factory factory, String wILD_VAR, Context context2, String klass, String path) {
    name = a.getName();
  ...
  public Predicate getPremises(
      List<String> list, List<String> newNames, CtElement elem, ErrorEmitter ee)
    ...
    for (int i = 0; i < invocationPredicates.size(); i++) {
      prem =
          Predicate.createConjunction(
              prem,
              Predicate.createEquals(
                  Predicate.createVar(newNames.get(i)), invocationPredicates.get(i)));
    }
    return prem.clone();
  }

  private List<Predicate> getPredicatesFromExpression(
      List<String> list, CtElement elem, ErrorEmitter ee) throws ParsingException {
