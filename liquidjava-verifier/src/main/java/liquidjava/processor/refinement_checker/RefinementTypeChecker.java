...
public class RefinementTypeChecker extends TypeChecker {
    ...
    if (localVariable.getAssignment() == null) {
      ...
      try {
        checkVariableRefinements(
            refinementFound, varName, localVariable.getType(), localVariable, localVariable);
  ...
  public <T, A extends T> void visitCtAssignment(CtAssignment<T, A> assignement) {
    ...
    if (ex instanceof CtVariableWriteImpl) {
      ...
      checkAssignment(
          name, varDecl.getType(), ex, assignement.getAssignment(), assignement, varDecl);
      ...
      checkAssignment(
          updatedVarName, cr.getType(), ex, assignement.getAssignment(), assignement, f);
  ...
  public <T> void visitCtLiteral(CtLiteral<T> lit) {
    ...
    if (types.contains(type)) {
      lit.putMetadata(
          REFINE_KEY,
          Predicate.createEquals(
              Predicate.createVar(WILD_VAR), Predicate.createLit(lit.getValue().toString(), type)));
  ...
  public <T> void visitCtField(CtField<T> f) {
    ...
    if (c.isPresent()) {
      ret =
          c.get().substituteVariable(WILD_VAR, nname).substituteVariable(f.getSimpleName(), nname);
    }
  ...
  public <T> void visitCtFieldRead(CtFieldRead<T> fieldRead) {
    ...
    if (context.hasVariable(fieldName)) {
      ...
      if (rv instanceof Variable
          && ((Variable) rv).getLocation().isPresent()
  ...
  public void visitCtIf(CtIf ifElement) {
    ...
    RefinedVariable freshRV =
        context.addInstanceToContext(freshVarName, factory.Type().INTEGER_PRIMITIVE, expRefs, exp);
  ...
  public <T> void visitCtArrayWrite(CtArrayWrite<T> arrayWrite) {
    ...
    try {
      fp =
          BuiltinFunctionPredicate.builtin_addToIndex(
              arrayWrite.getTarget().toString(),
              index.toString(),
              WILD_VAR,
              arrayWrite,
              getErrorEmitter());
  ...
  public <T> void visitCtConditional(CtConditional<T> conditional) {
    ...
    Predicate c =
        Predicate.createITE(
            cond,
            getRefinement(conditional.getThenExpression()),
  ...
  private void checkAssignment(
    ...
    // AQUI!!
    r.ifPresent(
        variableInstance -> vcChecker.removePathVariableThatIncludes(variableInstance.getName()));
  ...
  private <T> void getPutVariableMetadada(CtElement elem, String name) {
    Predicate cref =
        Predicate.createEquals(Predicate.createVar(WILD_VAR), Predicate.createVar(name));
    Optional<VariableInstance> ovi = context.getLastVariableInstance(name);
    if (ovi.isPresent()) {
      cref =
          Predicate.createEquals(
              Predicate.createVar(WILD_VAR), Predicate.createVar(ovi.get().getName()));
    }
