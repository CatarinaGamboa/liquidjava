...
public class AuxHierarchyRefinememtsPassage {
  ...
  private static HashMap<String, String> getParametersMap(
    ...
    for (int i = 0; i < fArgs.size(); i++) {
      String newName =
          String.format(tc.instanceFormat, fArgs.get(i).getName(), tc.getContext().getCounter());
  ...
  static void transferArgumentsRefinements(
    ...
    for (int i = 0; i < args.size(); i++) {
      ...
      Predicate superArgRef =
          superArg.getRefinement().substituteVariable(superArg.getName(), newName);
  ...
  static void transferReturnRefinement(
    ...
    if (functionRef.isBooleanTrue()) function.setRefinement(superRef);
      ...
      tc.checkStateSMT(
          ...
          superRef,
          method,
          "Return Refinement of Subclass must be subtype of the Return Refinement of the"
              + " Superclass");
  ...
  static Optional<RefinedFunction> functionInInterface(
    ...
    List<String> st =
        klass.getSuperInterfaces().stream()
      ...
      if (!subFunction.hasStateChange()) {
        ...
        for (int i = 0; i < superStates.size(); i++) {
          ...
          createVariableInContext(
              thisName, tc, subFunction, superFunction, method.getParameters().get(i));
          ...
          subConst =
              matchVariableNames(tc.THIS, thisName, superFunction, subFunction, subState.getTo());
    ...
    RefinedVariable rv =
        tc.getContext()
            .addVarToContext(
                ...
                ctParameter);
    rv.addSuperType(
        Utils.getType(superFunction.getTargetClass(), tc.getFactory())); // TODO: change: this only
    // works
    // for one superclass
