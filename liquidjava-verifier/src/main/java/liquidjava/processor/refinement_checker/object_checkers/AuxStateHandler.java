...
public class AuxStateHandler {
  ...
  public static void setDefaultState(RefinedFunction f, TypeChecker tc) {
    ...
    for (GhostFunction sg : sets) {
      if (sg.getReturnType().toString().equals("int")) {
        ...
        fail(
            "Ghost Functions not implemented for other types than int -> implement in"
  ...
  private static void setFunctionStates(
      RefinedFunction f,
      List<CtAnnotation<? extends Annotation>> anns,
      TypeChecker tc,
      CtElement element)
      throws ParsingException {
  ...
  private static ObjectState getStates(
      CtAnnotation<? extends Annotation> ctAnnotation,
      RefinedFunction f,
      TypeChecker tc,
      CtElement e)
      throws ParsingException {
    ...
    if (from == null
        && to != null) // has To but not From -> enters with true and exists with a specific state
    {
  ...
  private static Predicate createStatePredicate(
      String value, /* RefinedFunction f */
      String targetClass,
      TypeChecker tc,
      CtElement e,
      boolean isTo)
  ...
  /**
   * Create predicate with the equalities with previous versions of the object e.g.,
   * ghostfunction1(this) == ghostfunction1(old(this))
   *
   * @param p
   * @param th
   * @param sets
   ...
   * @return
   */
  private static Predicate addOldStates(
      Predicate p, Predicate th, List<GhostFunction> sets, TypeChecker tc) {
    Predicate c = p;
    for (GhostFunction gf : sets) {
      Predicate eq =
          Predicate.createEquals( // gf.name == old(gf.name(this))
  ...
  /**
   * If an expression has a state in metadata, then its state is passed to the last instance of the
  ...
  public static void updateGhostField(CtFieldWrite<?> fw, TypeChecker tc) {
    ...
    Optional<VariableInstance> invocation_callee =
        tc.getContext().getLastVariableInstance(parentTargetName);
    ...
    try {
      Predicate fromPredicate =
          createStatePredicate(stateChangeRefinementFrom, targetClass, tc, fw, false);
      Predicate toPredicate =
          createStatePredicate(stateChangeRefinementTo, targetClass, tc, fw, true);
      stateChange.setFrom(fromPredicate);
      stateChange.setTo(toPredicate);
    } catch (ParsingException e) {
      ErrorHandler.printCostumeError(
          fw,
          "ParsingException while constructing assignment update for `"
              + fw
              + "` in class `"
              + fw.getVariable().getDeclaringType()
              + "` : "
              + e.getMessage(),
          tc.getErrorEmitter());
    ...
    String newInstanceName =
        String.format(tc.instanceFormat, parentTargetName, tc.getContext().getCounter());
    ...
    VariableInstance vi2 =
        (VariableInstance)
            tc.getContext()
                .addInstanceToContext(newInstanceName, vi.getType(), vi.getRefinement(), fw);
  ...
  private static Predicate changeState(
    ...
    for (ObjectState stateChange : stateChanges) { // TODO: only working for 1 state annotation
      ...
      for (String s :
          map.keySet()) { // substituting function variables into annotation if there are any
    ...
    if (!found
        && !tc.getErrorEmitter()
            .foundError()) { // Reaches the end of stateChange no matching states
      String states =
          stateChanges.stream()
