...
public class MethodsFunctionsChecker {
  ...
  public <R> void getMethodRefinements(CtMethod<R> method) throws ParsingException {
    RefinedFunction f = new RefinedFunction();
    f.setName(
        method.getSimpleName().replaceAll("\\p{C}", "")); // remove any empty chars from string
   ...
   * @throws ParsingException
   */
  private Predicate handleFunctionRefinements(
      RefinedFunction f, CtElement method, List<CtParameter<?>> params) throws ParsingException {
    ...
    for (CtParameter<?> param : params) {
      ...
      RefinedVariable v =
          rtc.getContext().addVarToContext(param.getSimpleName(), param.getType(), c, param);
    ...
    if (ret.getReturnedExpression() != null) {
      ...
      if (method.getParent() instanceof CtClass) {
        RefinedFunction fi =
            rtc.getContext()
                .getFunction(
                    method.getSimpleName(), ((CtClass<?>) method.getParent()).getQualifiedName());
  ...
  public RefinedFunction getRefinementFunction(String methodName, String className, int size) {
    ...
    if (f == null)
      f =
          rtc.getContext()
              .getFunction(String.format("%s.%s", className, methodName), methodName, size);
    return f;
  }
  ...
  private void searchMethodInLibrary(CtExecutableReference<?> ctr, CtInvocation<?> invocation) {
    ...
    } else ctype = ctr.getDeclaringType().toString(); // missing

    String name = ctr.getSimpleName(); // missing
    if (rtc.getContext().getFunction(name, ctype) != null) { // inside rtc.context
      checkInvocationRefinements(
          invocation, invocation.getArguments(), invocation.getTarget(), name, ctype);
      return;
    } else {
      String prefix = ctype;
  ...
  private Map<String, String> checkInvocationRefinements(
    ...
    if (methodRef != null) {
      ...
      for (String s : vars)
        if (map.containsKey(s)) methodRef = methodRef.substituteVariable(s, map.get(s));
  ...
  private <R> Map<String, String> mapInvocation(
      List<CtExpression<?>> arguments, RefinedFunction f) {
    ...
    for (int i = 0; i < invocationParams.size(); i++) {
      ...
      if (iArg instanceof CtFieldRead) {
        ...
        Optional<VariableInstance> ovi =
            rtc.getContext().getLastVariableInstance(vr.getVariable().getSimpleName());
  ...
  private String createVariableRepresentingArgument(CtExpression<?> iArg, Variable fArg) {
    ...
    rtc.getContext()
        .addInstanceToContext(
            nVar, fArg.getType(), met.substituteVariable(rtc.WILD_VAR, nVar), iArg);
    return nVar;
  }
  ...
  public void loadFunctionInfo(CtExecutable<?> method) {
    ...
    if (className != null) {
      RefinedFunction fi =
          getRefinementFunction(method.getSimpleName(), className, method.getParameters().size());
      if (fi != null) {
        List<Variable> lv = fi.getArguments();
        for (Variable v : lv) rtc.getContext().addVarToContext(v);
      } else {
        assertFalse(
            "Method should already be in context. Should not arrive this point in refinement"
                + " checker.",
            true);
