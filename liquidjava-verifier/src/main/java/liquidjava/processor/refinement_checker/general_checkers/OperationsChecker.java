...
public class OperationsChecker {
  ...
  public <T> void getBinaryOpRefinements(CtBinaryOperator<T> operator) throws ParsingException {
    ...
    if (parent instanceof CtAssignment<?, ?>
      ...
      CtVariableWrite<?> parentVar =
          (CtVariableWrite<?>) ((CtAssignment<?, ?>) parent).getAssigned();
    ...
    if (type.contentEquals("boolean")) {
      ...
      if (parent instanceof CtLocalVariable<?>
          || parent instanceof CtUnaryOperator<?>
          || parent instanceof CtReturn<?>)
        operator.putMetadata(
            rtc.REFINE_KEY, Predicate.createEquals(Predicate.createVar(rtc.WILD_VAR), oper));
  ...
  public <T> void getUnaryOpRefinements(CtUnaryOperator<T> operator) throws ParsingException {
    ...
    if (ex instanceof CtVariableWrite) {
      ...
      rtc.checkVariableRefinements(
          all, name, w.getType(), operator, w.getVariable().getDeclaration());
      return;
    ...
    else newName = String.format(name, rtc.getContext().getCounter());
    ...
    if (p instanceof CtIf) all = opS;
    else
      all =
          Predicate.createEquals(
              Predicate.createVar(rtc.WILD_VAR), opS); // TODO SEE IF () IN OPS IS NEEDED
  ...
  /**
   * Retrieves all the refinements for the Operation including the refinements of all operands for
   * expressions without a variable as parent
   *
   * @param operator
   * @param element
   ...
   * @throws ParsingException
   */
  private Predicate getOperationRefinements(
      CtBinaryOperator<?> operator, CtVariableWrite<?> parentVar, CtExpression<?> element)
      throws ParsingException {
    ...
    if (element instanceof CtVariableRead<?>) {
      ...
      if (elemVar instanceof CtFieldRead) elemName = String.format(rtc.thisFormat, elemName);
      ...
      if (parent != null && !(parent instanceof CtIfImpl)) {
        ...
        RefinedVariable newVi =
            rtc.getContext().addVarToContext(newName, elemVar.getType(), newElem_ref, elemVar);
      ...
      String s =
          a.toString().replace("(", "").replace(")", "").replace("==", "").replace(" ", ""); // TODO
      // IMPROVE
      ...
      if (method == null) return getOperationRefinementFromExternalLib(inv, operator);
      ...
      return new Predicate(
          newName, inv, rtc.getErrorEmitter()); // Return variable that represents the invocation
    }
    return rtc.getRefinement(element);
    // TODO Maybe add cases
  }

  private Predicate getOperationRefinementFromExternalLib(
      CtInvocation<?> inv, CtBinaryOperator<?> operator) throws ParsingException {
   ...
   * @throws ParsingException
   */
  private <T> Predicate getRefinementUnaryVariableWrite(
      CtExpression<T> ex, CtUnaryOperator<T> operator, CtVariableWrite<T> w, String name)
      throws ParsingException {
    ...
    Predicate c =
        getOperatorFromKind(operator.getKind(), ex).substituteVariable(rtc.WILD_VAR, newName);
  ...
  private Predicate getOperatorFromKind(UnaryOperatorKind kind, CtElement elem)
      throws ParsingException {
    String ret = null;
    switch (kind) {
      case POSTINC:
