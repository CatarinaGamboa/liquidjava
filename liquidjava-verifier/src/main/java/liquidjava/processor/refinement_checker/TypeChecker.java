...
public abstract class TypeChecker extends CtScanner {
  ...
  public String[] implementedTypes = {
    "boolean", "int", "short", "long", "float", "double"
  }; // TODO add
  // types e.g., "int[]"
  ...
  public Optional<Predicate> getRefinementFromAnnotation(CtElement element)
  ...
  private void createStateGhost(
      String string, CtAnnotation<? extends Annotation> ann, String an, CtElement element) {
    GhostDTO gd = null;
    try {
      gd = RefinementsParser.getGhostDeclaration(string);
    } catch (ParsingException e) {
      ErrorHandler.printCostumeError(
          ann, "Could not parse the Ghost Function" + e.getMessage(), errorEmitter);
      return;
    }
    if (gd.getParam_types().size() > 0) {
      ErrorHandler.printCostumeError(
          ann,
          "Ghost States have the class as parameter "
              + "by default, no other parameters are allowed in '"
              + string
              + "'",
          errorEmitter);
      return;
  ...
  protected void getGhostFunction(String value, CtElement element) {
    try {
      ...
      if (f != null && element.getParent() instanceof CtClass<?>) {
        ...
        GhostFunction gh =
            new GhostFunction(f, factory, klass.getQualifiedName(), klass.getSimpleName());
  ...
  public void checkSMT(Predicate expectedType, CtElement element) {
    vcChecker.processSubtyping(
        expectedType, context.getGhostState(), WILD_VAR, THIS, element, factory);
    element.putMetadata(REFINE_KEY, expectedType);
  }

  public void checkStateSMT(
      Predicate prevState, Predicate expectedState, CtElement target, String string) {
    vcChecker.processSubtyping(
  ...
  public void createError(
      CtElement element, Predicate expectedType, Predicate foundType, String customeMessage) {
  ...
  public void createStateMismatchError(
