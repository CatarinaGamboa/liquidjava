...
public abstract class Expression {
  ...
  public Expression changeAlias(Map<String, AliasDTO> alias, Context ctx, Factory f)
      throws Exception {
    Expression e = clone();
    if (this instanceof AliasInvocation) {
      ...
      if (alias.containsKey(ai.name)) { // object state
        ...
        for (int i = 0; i < children.size(); i++) {
          ...
          if (!checks)
            throw new Exception(
                "Type Mismatch: Cannoy substitute "
                    + aliasExp
                    + " : "
  ...
  private void auxChangeAlias(Map<String, AliasDTO> alias, Context ctx, Factory f)
      throws Exception {
    if (hasChildren())
      for (int i = 0; i < children.size(); i++) {
        if (children.get(i) instanceof AliasInvocation) {
          ...
          if (ai.hasChildren())
            for (int j = 0; j < ai.children.size(); j++) {
              ...
              if (!checks)
                throw new Exception(
                    "Type Mismatch: Cannot substitute "
                        + varExp
                        + ":"
                        + varType
                        + " by "
